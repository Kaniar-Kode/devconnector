    ...
    Profile.findOne({ user: req.user.id })
        .then((profile) => {
            Post.findById(req.params.id)
                ...
                .catch((err) => {
                    res.status(404).json({ postNotFound: 'No post found nigga !' })
                })
        })
})
    ...
    Profile.findOne({ user: req.user.id })
        .then((profile) => {
            Post.findById(req.params.id)
                .then((post) => {
                    if (post.likes.filter((like) => like.user.toString() === req.user.id).length === 0) {
                        return res.status(400).json({ alreadyLiked: 'User not yet liked this post !' })
                    }
...
router.post('/comment/:id', passport.authenticate('jwt', { session: false }), (req, res) => {
    ...
    Post.findById(req.params.id)
        ...
        .catch((err) => {
            res.status(404).json({ postNotFound: 'No post found niggo !' })
        })
})
